  ...
  var rotatingSlider = function(selector, options) {
    ...
    function initSingleSlider($el, options) {
      ...
      var defaultOptions = {
        ...
        handleSpeedCoef: 6,
        easing: "ease",
        persMult: 2.2, // Increased from 1.6 to 2.2 for more zoom out
        handlePersMult: 3,
        scrollRotation: true,
        keysRotation: true,
...
$(document).ready(function() {
  ...
  rotatingSlider(".slider3d", {
    ...
    withControls: true // Keep controls in the initialization but hide with CSS
  });

  // Set up automatic rotation
  let currentSlide = 0;
  const totalSlides = 5; // Number of slides in the slider

  function rotateToNextSlide() {
    currentSlide = (currentSlide + 1) % totalSlides;

    // Find the rotaters
    const $rotaters = $(".slider3d__rotater, .slider3d__handle__rotater");

    // Calculate the angle
    const angle = 360 / totalSlides;
    const newAngle = currentSlide * angle;
    ...
    // Update active handle item
    $(".slider3d__handle__item").removeClass("active");
    $(".slider3d__handle__item").eq(currentSlide).addClass("active");
  }

  // Start automatic rotation
  setInterval(rotateToNextSlide, 3500); // Rotate every 3.5 seconds
});
